    ...
    const statsDisplay = [
        {
            ...
            icon: <svg className="w-8 h-8 md:w-10 md:h-10 text-slate-400 group-hover:text-[#6b4423] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
    ...
    return (
        <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-6 mb-6 md:mb-8">
            {statsDisplay.map((stat, index) => (
                <div
                    key={index}
                    className={`${stat.bgColor} border border-gray-200 shadow-sm p-2.5 md:p-6 rounded-xl md:rounded-2xl transition-all flex flex-col items-center md:items-start text-center md:text-left`}
                >
                    <div className="flex justify-between items-start mb-2 md:mb-4 w-full">
                        <div className="text-2xl md:text-4xl mx-auto md:mx-0">{stat.icon}</div>
                        {stat.highlight && (
                    ...
                    <h3 className="text-slate-500 text-[9px] md:text-sm font-medium leading-tight mb-1">{stat.title}</h3>
                    <p className={`text-xl md:text-3xl font-bold ${stat.highlight ? 'text-red-600' : 'text-slate-800'}`}>
                        {stat.value}
                    </p>
                </div>
            ))}
